<div id="livewire">
    ...
    <div class="m-actionsheet" id="action-languages" wire:ignore>
        <div style="position:relative">
            ...
            <div class="aui-product-text-content h-100">
                ...
                <div class="aui-product-text-content-list">
                    <div class="aui-product-text-content-list-ft">
                        <a class="border-0 d-flex {{ $key == app()->getLocale() ? 'active' : null }}" href="{{ LaravelLocalization::getLocalizedURL($key) }}">
                            <span>
            ...
            <div class="aui-product-text-content h-100">
                ...
                <div class="aui-product-text-content-list">
                    <div class="aui-product-text-content-list-ft">
                        <a class="border-0 d-flex {{ $currency->code == session('currency') ? 'active' : null }}" href="{{ route('currency.set', $currency->code) }}">
    ...
    <section class="aui-cart-content">
                                ...
                                <div class="aui-cart-box-list-text py-2">
                                    ...
                                    <div class="aui-cart-box-list-text-price">
                                        <div class="aui-cart-box-list-text-pri">
                                            ...
                                            @if ($product['prices'])
                                            {{-- <b class="price">
                                                {{ currency(min(array_column($product['prices'], 'price')), 'USD',
                                                session('currency')) }}
                                            </b> --}}
                                    ...
                                    <div class="aui-cart-box-list-text-price d-none">
                                        <div class="aui-cart-box-list-text-arithmetic">
                                            <a href="javascript:void(0);" class="minus" wire:click="qtyremove({{ $key }})">-</a>
                                            <input type="number" class="num" {{ $product['instock']> 0 ?: 'disabled' }} wire:model="products.{{ $key }}.qty" value="{{ $product['qty'] }}">
                                            <a href="javascript:void(0);" class="plus" wire:click="qtyadd({{ $key }})">+</a>
                                        </div>
                                    ...
                                    <div class="aui-cart-box-list-text-price">
                                        <a href="{{ route('front.cartremove', $product['id']) }}" wire:click.prevent="remove({{ $key }})">
        ...
        <table class="table table-sm">
            <tr>
                ...
                <td>
                    ...
                    <input wire:model.debounce.1000ms="coupon" type="text" class="form-control form-control-sm" placeholder="{{ __('Coupon code') }}">
                    @if ($coupon_message)
                ...
                <td>
                    <select wire:model="shipping_fee" class="form-control form-control-sm select2">
                        ...
                        <option value="{{ $item->id }}" {{ $loop->first ? 'selected' : null }}>
                            {{ __(trim($item->name)) }} -
                            {{ $item->packing_charge_type == 'fixed'? currency($item->packing_charge, 'USD',
                            session('currency')): $item->packing_charge . '%' }}
                        </option>
                        @endforeach
                    </select>
                </td>
        ...
        <div class="aui-payment-bar border-top">
            @auth
            <button wire:click.prevent="togglecheckout" class="settlement w-100">{{ __('Checkout') }}</button>
            @else
            <button class="settlement w-100" onclick="location.href=`{{ route('login') }}`">{{ __('Login') }}</button>
            @endauth
        </div>
        @endif
            ...
            <div class="aui-product-text-content">
                ...
                <div class="px-2 border rounded">
                    <div class="aui-product-text-content-list">
                        <div class="aui-product-text-content-list-ft">
                            <span>
                                ...
                                <span class="text-danger text-xs">*</span>
                                @endif --}}
                                <select class="form-control select2" wire:model="province" wire:change="province" data-placeholder="{{ __('Province') }}">
                    ...
                    <div class="aui-product-text-content-list">
                        <div class="aui-product-text-content-list-ft">
                            <span>
                                <label for="district" class="form-control-label d-none">{{ __('District') }}</label>
                    ...
                    <div class="aui-product-text-content-list">
                        <div class="aui-product-text-content-list-ft">
                            <span>
                                <label for="commune" class="form-control-label d-none">{{ __('Commune') }}</label>
        ...
        <div class="aui-product-text-content-list">
            <div class="aui-product-text-content-list-ft">
                <span>
                    ...
                    @endif
                    @if ($commune)
                    <a href="#" class="float-right" wire:click.prevent="samelocation">{{ __('Same Location')
                                }}</a>
                    @endif
        ...
        <div class="aui-product-text-content-list">
            <div class="aui-product-text-content-list-ft">
                <span>
                    ...
                    <span class="text-danger text-xs">*</span>
                    @endif
                    {!! Form::text('phone', auth()->user()->phone, ['class' => 'form-control', 'wire:model' =>
                    'phone']) !!}
                    @error('phone')
                    <div class="error-feedback d-block">
                        {{ $message }}
            ...
            <div class="aui-product-text-content-list-ft">
                <span>
                    ...
                    <div class="input-group">
                        <input class="form-control" placeholder="{{ __('Latitude') }}" wire:model="latitude" type="text">
                        <input class="form-control" placeholder="{{ __('Longitude') }}" wire:model="longitude" type="text">
                    </div>

                    @if ($latitude && $longitude)
                    <iframe src="https://maps.google.com/maps?q={{ $latitude }},{{ $longitude }}&hl={{ app()->getLocale() }}&t=&z=15&ie=UTF8&iwloc=&output=embed" width="100%" height="100" class="border"></iframe>
                    @endif
                </span>
            </div>
        </div>
                ...
                <span>
                    ...
                    <div class="form-row">
                        <div class="col-5 col-xl-6">
                            <select class="form-control select2-image" wire:model="payment_via" data-minimum-results-for-search="Infinity">
                        ...
                        <div class="col col-xl-6">
                            <label class="btn m-0 p-2 form-control" for="payment_image">
                                {{ __('Upload Payment')}}
                            </label>
                            <input type="file" class="form-control d-none" accept="image/*" id="payment_image">
    ...
    <a href="javascript:void(0);" class="actionsheet-action" id="cancel" wire:click.prevent="togglecheckout"></a>
    @if ($payment_image)
...
@endif
<div wire:loading wire:target="payment_image" id="loading-payment_image">
    <div class="swal2-container swal2-center swal2-fade swal2-shown" class="swal2-popup swal2-toast swal2-show swal2-loading" style="display: flex;">
        ...
        <div class="swal2-actions swal2-loading" style="display: flex;">
            <div class="swal2-confirm swal2-styled" style="border-left-color: var(--primary); border-right-color: var(--primary); display: flex;">
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on('change', `#payment_image`, function(e) {
        $(`#loading-payment_image`).addClass('d-flex');
        ...
        , }).then((output) => {
            ...
            $.ajax({
                ...
                , success: (res) => {
                    ...
                    $(`#loading-payment_image`).removeClass('d-flex');
                }
            });
            $(e.target).val('');
        });
